...
public class ThingsManageExecutor extends ThingsEngineBaseExecutor implements ThingsManageEngine {
    ...
    private void findThingsFilters(ThingsContainer thingsContainer, ThingsFilter thingsFilter, Object bean) {
        ...
        String identifier = StrUtil.isNotBlank(thingsFilter.identifier()) ? thingsFilter.identifier() : THINGS_WILDCARD;
        String productCode = StrUtil.isNotBlank(thingsFilter.productCode()) ? thingsFilter.productCode() : THINGS_WILDCARD;
        Set<ThingsFilters> filters = THINGS_FILTERS_TABLE.get(identifier, productCode);
        if (filters == null) {
            filters = new ConcurrentHashSet<>();
        }
        filters.add(new ThingsFilters(thingsFilter, (Filter) bean));
    ...
    private void findThingsInterceptors(ThingsContainer thingsContainer, ThingsInterceptor thingsInterceptor, Object bean) {
        ...
        String identifier = StrUtil.isNotBlank(thingsInterceptor.identifier()) ? thingsInterceptor.identifier() : THINGS_WILDCARD;
        String productCode = StrUtil.isNotBlank(thingsInterceptor.productCode()) ? thingsInterceptor.productCode() : THINGS_WILDCARD;
        Table<String, String, Set<ThingsInterceptors>> table = thingsInterceptor.request() ? THINGS_REQUEST_INTERCEPTORS_TABLE : THINGS_RESPONSE_INTERCEPTORS_TABLE;
        Set<ThingsInterceptors> interceptors = table.get(identifier, productCode);
        if (interceptors == null) {
            interceptors = new ConcurrentHashSet<>();
        }
        interceptors.add(new ThingsInterceptors(thingsInterceptor, (Interceptor) bean));
