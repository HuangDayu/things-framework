...
public class ThingsInputtingIntercepting implements ThingsIntercepting {
    ...
    public boolean preHandle(ThingsRequest thingsRequest, ThingsResponse thingsResponse) {
        // 如果消息为空，则直接返回false
        if (thingsRequest == null || thingsRequest.getTrm() == null) {
            return false;
        }
        // 如果是回复，则将其加入异步处理，不进行下一步处理
        if (thingsResponse.getTrm() != null && !hasAsyncRequest(thingsRequest.getTrm().getId())) {
            return false;
        }
        // 如果产品码为空，则直接返回false
        return !StrUtil.isBlank(thingsRequest.getTrm().getMessageMethod().getProductCode());
    }
