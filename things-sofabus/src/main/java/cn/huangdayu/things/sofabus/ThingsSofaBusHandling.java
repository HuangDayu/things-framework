...
import static cn.huangdayu.things.sofabus.ThingsSofaBusManager.createTopic;
...
public class ThingsSofaBusHandling implements ThingsHandling {
    ...
    public void doHandle(ThingsRequest thingsRequest, ThingsResponse thingsResponse) {
        ...
        String topic = createTopic(thingsRequest.getJtm());
        for (ThingsSofaBus bus : thingsSofaBus) {
            if (bus.isStarted()) {
                bus.output(topic, thingsRequest, thingsResponse);
            }
        }
    }
}
