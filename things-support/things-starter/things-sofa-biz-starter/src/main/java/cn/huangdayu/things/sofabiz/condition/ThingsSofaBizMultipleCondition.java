...
public class ThingsSofaBizMultipleCondition implements Condition {
    @Override
    public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
        return Objects.requireNonNull(context.getBeanFactory()).getBeansWithAnnotation(SpringBootApplication.class).values().stream().map(Object::getClass)
                .noneMatch(clazz -> clazz.isAnnotationPresent(EnableThingsSofaBiz.class) && clazz.getAnnotation(EnableThingsSofaBiz.class).singletonMode());
    }
}
