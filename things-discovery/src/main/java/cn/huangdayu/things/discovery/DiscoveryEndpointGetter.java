...
public class DiscoveryEndpointGetter implements ThingsEndpointGetter {
    ...
    public Set<String> getPublishUris(JsonThingsMessage thingsMessage) {
        ...
        if (thingsMessage.getMethod().startsWith(ThingsConstants.Methods.EVENT_LISTENER_START_WITH)) {
            Set<ThingsInstance> consumeInstances = thingsInstancesManager.getConsumeInstances(baseMetadata.getProductCode(), baseMetadata.getDeviceCode(), thingsMessage.getMethod());
        ...
        if (StrUtil.isNotBlank(thingsInstancesManager.getThingsInstance().getBrokerUri())) {
            set.add(thingsInstancesManager.getThingsInstance().getBrokerUri());
        }
        return set;
    }
    ...
    public String getInvokeUri(JsonThingsMessage thingsMessage) {
        ...
        return thingsInstancesManager.getThingsInstance().getBrokerUri();
    }
